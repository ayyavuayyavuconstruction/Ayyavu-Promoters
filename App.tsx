...
import { MOCK_PROJECTS, SQFT_TO_CENTS } from './constants';
...
const STORAGE_KEY = 'estatenexus_projects_data_v2';
const COMPANY_STORAGE_KEY = 'estatenexus_company_settings';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem(COMPANY_STORAGE_KEY, JSON.stringify(companySettings));
  }, [companySettings]);
  ...
  // Aggregated Financial Calculations for the Sidebar
  ...
  const handleCreateProject = (data: Omit<Project, 'id' | 'sites'>) => {
    const newProject: Project = { ...data, id: `p-${Date.now()}`, sites: [], imageUrls: data.imageUrls || [] };
    setProjects(prev => [...prev, newProject]);
    setSelectedProjectId(newProject.id);
    setIsCreateProjectModalOpen(false);
  };

  const handleUpdateProjectDetails = (data: Omit<Project, 'id' | 'sites'>) => {
  ...
  const handleUpdateSite = useCallback((siteId: string, updates: Partial<Site>) => {
    ...
    setProjects(prevProjects => prevProjects.map(project => {
      if (project.id === selectedProjectId) {
        return {
          ...project,
          sites: project.sites.map(s => s.id === siteId ? { ...s, ...updates } : s)
        };
      }
      return project;
    }));
  ...
  const handleAddSite = (newSiteData: Omit<Site, 'id'>) => {
    if (!selectedProjectId) return;
    const newSite: Site = { ...newSiteData, id: `${selectedProjectId}-${Date.now()}` };
    setProjects(prevProjects => prevProjects.map(project => {
      if (project.id === selectedProjectId) {
        return { ...project, sites: [...project.sites, newSite] };
      }
      return project;
    }));
  ...
  const handleDeleteSite = useCallback((siteId: string) => {
    ...
    setProjects(prevProjects => prevProjects.map(project => {
      if (project.id === selectedProjectId) {
        return { ...project, sites: project.sites.filter(s => s.id !== siteId) };
      }
      return project;
    }));
    setSelectedSiteId(null);
  ...
  return (
    <div className="min-h-screen flex flex-col print:bg-white bg-slate-50">
        ...
        {!selectedProject ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 h-full">
            ...
            <div className="lg:col-span-4 space-y-6 overflow-y-auto pb-10 scrollbar-hide">
              ...
              {/* Aggregated Financial Summary Section */}
      ...
      {isCompanyModalOpen && <CompanySettingsModal settings={companySettings} onSave={(s) => { setCompanySettings(s); setIsCompanyModalOpen(false); }} onClose={() => setIsCompanyModalOpen(false)} />}
